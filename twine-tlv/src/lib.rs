    ...
    T: TlvMetadata + TryEncodeTlvValue,
{
    ...
    let _ = value.try_encode_tlv_value(buffer);

    Ok(original_remaining - buffer.remaining_mut())
}
...
mod tests {
    ...
    use twine_macros::ConstantTlv;

    #[derive(Clone, ConstantTlv, Copy, Debug, Eq, PartialEq)]
