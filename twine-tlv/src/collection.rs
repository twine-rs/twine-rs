...
impl<const CAPACITY: usize> TlvCollection<CAPACITY> {
    /// Creates a new, empty `TlvBuffer` with a length of 0.
    pub fn new() -> Self {
        Self {
            buffer: [0; CAPACITY],
        }
    ...
    fn find_tlv<T>(tlv_type: T, buffer: impl AsRef<[u8]>) -> Option<Range<usize>>
    where
        T: Into<u8> + Copy,
    {
        let buffer = buffer.as_ref();
    ...
    {
        if let Some(range) = Self::find_tlv(T::TLV_TYPE, self.buffer) {
...
mod tests {
    ...
    fn success_find_tlv() {
        ...
        let tlv = TlvCollection::<15>::find_tlv(0, buffer);
    ...
    #[test]
    fn success_find_tlv_returns_none_unknown_tlv() {
        let buffer = [
            0x01, 0x02, 0x03, 0x04, 0x02, 0xFF, 0x00, 0x02, 0xAA, 0xAA, 0x00, 0x03, 0x00, 0x00,
            0x16,
        ];
        let tlv = TlvCollection::<15>::find_tlv(0xFF, buffer);
        assert_eq!(tlv, None);
    }

    #[test]
    fn success_find_tlv_returns_none_on_empty_buffer() {
        log_init();
        let buffer = [0_u8; 15];
        let tlv = TlvCollection::<15>::find_tlv(0xFF, buffer);
        assert_eq!(tlv, None);
    ...
    fn success_try_encode_tlv() {
        ...
        let mut tlv_collection = TlvCollection::<CAPACITY>::new();
        tlv_collection
            .push(tlv_data)
