...
use quote::quote;
use syn::{parse_macro_input, DeriveInput, Expr, ExprLit, Generics, Ident, Lit, Meta};
...
/// Derive entry point for `#[derive(Tlv)]`.
...
impl<'a> DeriveCtx<'a> {
    ...
    fn impl_tlv_type(&self, target: &Ident, tlv_type: u8) -> TokenStream2 {
    ...
    fn impl_tlv_metadata(&self, target: &Ident) -> TokenStream2 {
        let (ig, tg, wc) = self.split_generics();
        quote! {
    ...
    fn impl_constant_length(&self, target: &Ident) -> TokenStream2 {
        let (ig, tg, wc) = self.split_generics();
    ...
    fn impl_decode_tlv_unchecked(&self, target: &Ident) -> TokenStream2 {
        let (ig, tg, wc) = self.split_generics();
        quote! {
    ...
    fn impl_try_encode_tlv(&self, target: &Ident) -> TokenStream2 {
        let (ig, tg, wc) = self.split_generics();
        quote! {
    ...
    fn impl_ref_impls(&self, target: &Ident) -> TokenStream2 {
        let (ig, tg, wc) = self.split_generics();
    ...
    fn impl_derive_inner(&self, target: &Ident, is_variant: bool) -> TokenStream2 {
    ...
    fn impl_variant_struct(&self, variant_ident: &Ident) -> TokenStream2 {
        let base = self.ident;
        let vis = self.vis;
    ...
    fn impl_variant_encode_decode(&self, variant_ident: &Ident) -> TokenStream2 {
        let base = self.ident;
        let (ig, tg, wc) = self.split_generics();

        // Don't generate these if `derive_inner` will provide them.
        // (The caller gates `impl_derive_inner` separately.)
        quote! {
