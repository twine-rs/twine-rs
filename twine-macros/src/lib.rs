    ...
    let output = quote! {
        ...
        impl TryEncodeTlv for #ident {
            fn try_encode_tlv(&self, buffer: &mut [u8]) -> Result<usize, TwineTlvError> {
                ...
                if buffer.len() < Self::tlv_total_constant_len() {
                    return Err(TwineTlvError::BufferEncodeTooShort);
                }

                buffer.put_u8(Self::TLV_TYPE);
                buffer.put_tlv_length(Self::TLV_LEN);

                let _ = self.try_encode_tlv_value(buffer)?;

                Ok(self.tlv_total_len())
            }
        }
    };
    ...
    let output = quote! {
        ...
        impl TryEncodeTlv for #ident {
            fn try_encode_tlv(&self, buffer: &mut [u8]) -> Result<usize, TwineTlvError> {
                ...
                if buffer.len() < self.tlv_total_len() {
                    return Err(TwineTlvError::BufferEncodeTooShort);
                }
