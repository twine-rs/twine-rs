...
impl DevicePool {
    ...
    pub fn device_count(&self) -> usize {
        self.inner.lock().expect("pool lock poisoned").devices.len()
    }
}
...
mod tests {
    ...
    fn reserved_devices_lists_held_ids_and_capabilities() {
        ...
        assert_eq!(entries, vec![(dev_id("x"), FFD), (dev_id("y"), RFD)]);
    }
    ...
    fn request_by_capability_excluding_empty_list_behaves_like_plain() {
        ...
        let mut handle = pool.handle();
        let id = handle.request_by_capability_excluding(FFD, &[]).unwrap();
        assert_eq!(id, dev_id("ffd-1"));
    }
